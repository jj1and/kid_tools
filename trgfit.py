...
class Oneshot():
    ...
    def set_init_phase_params(self, **kwargs):
        ...
        init_phase_bias = np.mean(self.phase[0:50])
        #bias_sigma = np.std(self.phase[0:10])
        std_phase_sum = np.sum(self.phase[0:5])
        if(options['fit_range']==True):
            ...
            init_phase_Amp = np.max(np.abs(init_phase_bias - self.phase[:int(len(self.time)/2)]))
            phase_start_index = np.argmax(np.abs(init_phase_bias - self.phase[:int(len(self.time)/2)]))
            #phase_stop_index = int(np.max(np.where(self.phase[phase_start_index:]>=(init_phase_bias-init_phase_Amp/(np.e**4)+2*bias_sigma)))/2) + phase_start_index
            phase_stop_index = int(((len(self.time)-1)-phase_start_index)/2) + phase_start_index
            ...
            ('phase_tau', init_phase_tau, True, 1e-6, 1e9, None, None),
...
class Trgholder():
    ...
    def analyze_trg(self, trg_set, **kwargs):
        ...
        for one_trg in trg_set:
            ...
            if(np.abs(tmp_phase_sum/tmp_std_phase_sum)<1):
                self.failed_list.append(tmp_shot)
            elif(tmp_shot.lmfit_tau_result.ier in (1, 2, 3)):
                self.oneshot_list.append(tmp_shot)
                print(tmp_shot.lmfit_tau_result.message)
                if((tmp_shot.lmfit_tau_result.errorbars)==True):
